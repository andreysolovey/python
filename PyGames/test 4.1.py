...
class Ground(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.transform.scale(grass_img, (690, 75))
        self.rect = self.image.get_rect()
        self.rect.y = 275
        self.rect.x = 0
        self.speed = 0
...
class Bat(pygame.sprite.Sprite):
    def __init__(self, speed=6.5, sizex=-1, sizey=-1):
        pygame.sprite.Sprite.__init__(self, self.containers)
        self.images = bat_img
        self.images.set_colorkey(black)
        self.rect = self.images.get_rect()
        self.bat_height = [height*0.55, height*0.51, height*0.45, height*0.48]
...
background = pygame.transform.scale(background_img, (690, 350))
...
def gameplay():
    ...
    while not gameQuit:
        ...
        while not gameOver:
            ...
            hits = pygame.sprite.spritecollide(enemy, alien, True, pygame.sprite.collide_circle)
            for hit in hits:
                hit.movement[0] = -1*gamespeed
                if pygame.sprite.spritecollide(alien, hit, True):
                    alien.shield -= 30
                    if alien.shield <= 0:
                        alien.hide()
                        alien.isDead = True
            ...
            hits = pygame.sprite.spritecollide(enemy, bat, True, pygame.sprite.collide_circle)
            for hit in hits:
                hit.movement[0] = -1*gamespeed
                if pygame.sprite.spritecollide(alien, hit, True):
                    alien.shield -= 30
            ...
            if len(bat) == 0 and random.randrange(0, 200) == 10 and counter > 500:
                for l in last_obstacle:
                    if l.rect.right < width*0.8:
                        last_obstacle.empty()
                        last_obstacle.add(Bat(gamespeed, 46, 40))
            enemy.update()
            bullets.update()
            bat.update()
            all_sprites.update()
